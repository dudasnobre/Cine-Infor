/** JavaScript da interface do Cine Infor. */
    /* =========================
       TRAILER (fallback da miniatura)

       Nem todo vídeo do YouTube tem a versão "maxresdefault" da
       miniatura. Se ela não existir, usamos a "hqdefault", que
       sempre existe, para a capa nunca aparecer quebrada.
    ========================= */

    const trailerThumb =
      document.getElementById('trailerThumb');

    if (trailerThumb) {
      trailerThumb.addEventListener('error', function () {
        const fallback = 'https://img.youtube.com/vi/N_-rLOuM54Q/hqdefault.jpg';
        if (trailerThumb.src !== fallback) {
          trailerThumb.src = fallback;
        }
      }, { once: true });
    }
    ...
    /*
      Pipoca disponível somente
      salgada. Aqui só controlamos
      a quantidade escolhida.
    */

    function atualizarQuantidadePipoca() {
    ...
    */
    ...
    const URL_APPS_SCRIPT =
      'https://script.google.com/macros/s/AKfycbxP-LhPWlwKQXJHt0s4OO4C42kdj-L087Uyq-IXuS8kOU8_5BBKe9pLyiriJ-88rtVNSQ/exec';
    ...
    form.addEventListener('submit', async function (event) {
      ...
      const tiposPipoca = pipocaSim.checked
        ? new Array(qtdPipoca).fill('Salgada')
        : [];
    ...
    btnNovoPedido.addEventListener('click', function () {
      ...
      form.querySelectorAll('input[name="pipoca"]')
